...
if ! node=$(curl --silent -X 'GET' 'http://192.248.164.239:10973/infos/self-clique'); then
...
addresses=$(curl --silent -X 'GET' 'http://192.248.164.239:10973/miners/addresses' | jq '.addresses')
if [ -z "$addresses" ]; then
